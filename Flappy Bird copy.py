...
# Bird_Normal = pygame.image.load("Animation_Normal.png") # Нормальная позиция
# Bird_Fall = pygame.image.load("Animation_Fall.png") # Падение                   Правильно ли?
# Bird_Go_Up = pygame.image.load("Animation_Go_Up.png") # Взлёт
# Птица
...
def draw_text(game_score, font, black, x, y):
    ...
    screen.blit(game_score_numbers, (400, 25))
...
while a:
    ...
    if len(tube_group) > 0:
        if bird_group.sprites()[0].rect.left > tube_group.sprites()[0].rect.left and bird_fly_through == False and bird_group.sprites()[0].rect.right < tube_group.sprites()[0].rect.right:
            bird_fly_through = True
        if bird_fly_through == True:
